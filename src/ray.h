...
// return: the orthogonal vector connecting vec to the line spanned by line_vec.
vec4 orthogonal(vec4 vec, vec4 line_vec) {
...
vec3 deflected_3d(vec3 ray, float angle, vec3 a, vec3 b) {
    ...
    vec3 y = glm::cross(axis, ray) * sin(angle);
    vec3 z = axis * glm::dot(axis, ray) * (1 - cos(angle));
...
vec4 deflected(vec4 ray, float angle, vec4 a, vec4 b) {
    vec3 ray_rot = deflected_3d(vec3(ray), angle, vec3(b), vec3(b));
